<template>
  <q-card class="congeCardCreateB absenceCardCreateB absenceCardCreateNotPaid">
        <q-card-title v-if="$route.params.isadmin != 'rh'">
          {{$t('demande.absence.absencenotpaiddemende')}}
        </q-card-title>
        <q-card-title v-else>{{$t('demande.absence.absencenotpaiddemendecc')}}</q-card-title>
        ...
        <q-card-main>
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="marginsmedium"></div>
              <div class="row">
                <div class="col-md-4">
                  <q-input v-model="motif" placeholder="Motif" />
                </div>
              </div>
            </div>
          </div>
...
export default Vue.extend({
  ...
  methods: {
    senddemande (confirmlabel) {
      Dialog.create({
        ...
        if (this.collaborateur !== null) {
          services.notpaidstore({ dateStart: moment(this.dateStart).format('DD-MM-YYYY'), dateEnd: moment(this.dateEnd).format('DD-MM-YYYY'), morning: !this.morning, afternoon: this.afternoon, motif: this.motif, colabId: this.collaborateur.value })
        } else if (this.$route.params.isadmin !== 'rh') {
          services.notpaidstore({ dateStart: moment(this.dateStart).format('DD-MM-YYYY'), dateEnd: moment(this.dateEnd).format('DD-MM-YYYY'), morning: !this.morning, afternoon: this.afternoon, motif: this.motif })
        } else {
          Notify.create({
            type: 'negative',
            icon: 'fa fa-times',
    ...
    storeFunc () {
      if (this.$store.state.Auth.user.isBlocked === 'true') {
      ...
      } else if (moment().diff(moment(this.dateStart), 'days') > 0 && this.$route.params.isadmin !== 'rh') {
        Notify.create({
          type: 'negative',
          icon: 'fa fa-times',
        ...
        Notify.create({
          type: 'negative',
          icon: 'fa fa-times',
          message: this.$t('demande.absence.emptymotifcreatevalidation'),
          color: 'white'
        })
      } else {
        if (this.daysNumber > 0) {
          if (this.daysNumber > 4 && this.$route.params.isadmin !== 'rh') {
            Notify.create({
              type: 'negative',
              icon: 'fa fa-times',
              message: this.$t('demande.absence.morethanfour'),
              color: 'white'
            })
          } else {
            ...
            services.getDurationAC({ type: 'absencenonremunere', dateStart: moment(this.dateStart).format('DD-MM-YYYY'), dateEnd: moment(this.dateEnd).format('DD-MM-YYYY'), morning: !this.morning, afternoon: this.afternoon })
          }
        } else {
          Notify.create({
            type: 'negative',
    ...
    showDateDiff (initIterr = false) {
      ...
      if (initIterr !== true) {
        if (moment().diff(moment(this.dateStart), 'days') > 0 && this.$route.params.isadmin !== 'rh') {
          Notify.create({
            type: 'negative',
            icon: 'fa fa-times',
            message: this.$t('demande.absence.dateInPasteERR'),
          ...
          if (this.daysNumber >= 0 && this.daysNumber <= 4) {
            // let textMessage = ''
