<template>
  <q-card class="congeCardCreateB absenceCardCreateM" :class="{absenceCardCreateM: $route.params.isadmin != 'rh', absenceCardCreateMcc: $route.params.isadmin == 'rh'}">
        <q-card-title v-if="$route.params.isadmin != 'rh'">
          {{$t('demande.absence.absencematernitydemende')}}
        </q-card-title>
        <q-card-title v-else>{{$t('demande.absence.absencematernitydemendecc')}}</q-card-title>
    ...
    senddemande (confirmlabel) {
      Dialog.create({
        ...
        if (this.collaborateur !== null) {
          services.maternitystore({ dateStart: moment(this.dateStart).format('DD-MM-YYYY'), dateEnd: moment(this.dateEnd).format('DD-MM-YYYY'), morning: !this.morning, afternoon: this.afternoon, colabId: this.collaborateur.value })
        } else {
          Notify.create({
            type: 'negative',
            icon: 'fa fa-times',
    ...
    storeFunc () {
      if (this.$store.state.Auth.user.isBlocked === 'true') {
        ...
        if (this.daysNumber > 0) {
          ...
          services.getDurationAC({ type: 'absencematernite', dateStart: moment(this.dateStart).format('DD-MM-YYYY'), dateEnd: moment(this.dateEnd).format('DD-MM-YYYY'), morning: !this.morning, afternoon: this.afternoon })
        } else {
          Notify.create({
            type: 'negative',
            icon: 'fa fa-times',
